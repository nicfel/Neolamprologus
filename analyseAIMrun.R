...
for (i in seq(1, length(unique_ranked$topology))){
  ...
  if (exists("arrows.data")){
    p <- p + geom_segment(data=arrows.data,  aes(x=x, xend=xend,y = y, yend=yend), arrow = arrow(angle=10,type="closed",ends="last"), size = 5, alpha=0.5)
  }
  p <- p + geom_tiplab()
